 ...
 *
 * Khac ban task-connect-fe: khong co man rieng nhap OTP (do khong dung chung voi luong xac
 * minh dang ky nao ca o app nay) - dua thang sang /dat-lai-mat-khau, man do gom ca OTP lan
 * mat khau moi trong cung mot form, khop dung 1 lan goi /auth/reset-password cua backend.
 */
export function ForgotPasswordPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      navigate('/dat-lai-mat-khau', { state: { email: email.trim() }, replace: true })
    } catch (err) {
      ...
      <div className="flex flex-col gap-6" onKeyDown={submitOnEnter(handleSubmit, busy)}>
        <div className="flex flex-col gap-2">
          ...
          <p className="m-0" style={{ color: 'var(--text-muted)' }}>
            Nhập email của tài khoản Admin. Chúng tôi gửi mã 6 chữ số để bạn đặt lại mật khẩu.
          </p>
        </div>
